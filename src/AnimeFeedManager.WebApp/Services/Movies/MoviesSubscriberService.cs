...
public class MoviesSubscriberService : IMoviesSubscriberService
{
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("api/Movies/subscriptions", new ShortSeriesSubscriptionDto(subscriber, series, notificationDate), cancellationToken: cancellationToken);
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("api/Movies/subscriptions/unsubscribe", new ShortSeriesUnsubscribeDto(subscriber, series), cancellationToken: cancellationToken);
        await response.CheckForProblemDetails();
    }
}