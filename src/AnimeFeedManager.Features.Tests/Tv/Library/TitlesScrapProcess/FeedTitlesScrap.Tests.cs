...
public class FeedTitlesScrapTests
{
    ...
    public async Task Should_Update_Only_Series_Without_Feed_With_Matches()
    {
        ...
        var s2 = MakeSeries("2", "Other Title", string.Empty, SeriesStatus.NotAvailable(),
            altTitles: "Magic Academy"); // update by alt title
        ...
        {
            var updated = d.FeedTitleUpdateInformation.Where(x => x.UpdateStatus == UpdateStatus.Updated)
                .Select(x => x.Series.RowKey).OrderBy(x => x).ToArray();
    ...
    public async Task Should_Not_Update_When_No_Matches_Or_Already_Ongoing()
    {
        ...
        result.AssertOnSuccess(d =>
            Assert.All(d.FeedTitleUpdateInformation, x => Assert.Equal(UpdateStatus.NoChanges, x.UpdateStatus)));
    }
    ...
    public async Task Should_Create_Events_Only_For_Updated_Series()
    {
        ...
        var result = await Task
            .FromResult(Result<FeedTitleUpdateData>.Success(new FeedTitleUpdateData(season, feedTitles, [])))
            .UpdateSeries(getter, updater, CancellationToken.None)
