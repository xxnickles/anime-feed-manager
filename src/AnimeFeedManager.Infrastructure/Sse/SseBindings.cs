...
/// call binds one CLR event type to one SSE event name, an <see cref="Audience"/>, and a render
/// step. The builder produces an immutable list consumed by <see cref="SseStream"/>.
/// </summary>
public sealed class SseBindings
{
    ...
    /// <summary>
    /// Bind <typeparamref name="TEvent"/> to <paramref name="eventName"/>, rendering it through
    /// <typeparamref name="TComponent"/> via <see cref="HtmlRenderer"/>. The event is passed to
    /// the component as its <c>Model</c> parameter, matching the app's existing
    /// <c>RazorComponentResult&lt;TForm&gt;</c> rendering convention. A fresh <see cref="HtmlRenderer"/>
    /// is constructed and disposed per render, using the connection's own scoped
    /// <see cref="IServiceProvider"/> (see <see cref="SseStream"/>) — matching the framework's own
    /// non-request rendering samples, no artificial scope is created here.
    /// </summary>
    public SseBindings AddHtml<TEvent, TComponent>(string eventName, Audience audience)
        where TEvent : notnull
