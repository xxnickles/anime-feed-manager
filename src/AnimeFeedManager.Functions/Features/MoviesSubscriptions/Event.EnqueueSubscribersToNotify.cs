...
public class EnqueueSubscribersToNotify
{
    ...
    [Function("EnqueueMoviesSubscribersToNotify")]
    [QueueOutput(QueueNames.MoviesSubscriptionsToProcess, Connection = "AzureWebJobsStorage")]
    ...
    )
    {
        ...
        return subscribers.Match(
            subs =>
            {
                _logger.LogInformation("Processing following Movies subscribers {Subscribers}", string.Join(", ", subs.Select(x => x.Subscriber)));
