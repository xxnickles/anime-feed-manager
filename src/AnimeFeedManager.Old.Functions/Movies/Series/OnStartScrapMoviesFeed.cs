...
public class OnStartScrapMoviesFeed
{
    ...
    {
        return Task.WhenAll(seasons.Select((season, index) => _domainPostman.SendDelayedMessage(new ScrapMoviesSeasonFeed(season), new Delay(TimeSpan.FromSeconds(index * 15)), token)))
            .FlattenResults()
