    ...
    ILogger<NyaaCollectionJob> logger)
{
    ...
    public async Task Run(CancellationToken cancellationToken)
    {
        ...
        var index = await BuildTitleIndex(errors, cancellationToken);
        var counts = index is null
            ? default
            : await LoadAndProcessFeed(index, errors, cancellationToken);

        await PersistRun(startedAt, counts, errors, cancellationToken);
    }

    private Task<LibraryTitleIndex?> BuildTitleIndex(List<string> errors, CancellationToken cancellationToken) =>
        _resolveCurrentSeason(cancellationToken)
            ...
            .MatchToValue(
                index => (LibraryTitleIndex?)index,
                error =>
                {
                    errors.Add($"Failed to build title index: {error.Message}");
                    return null;
                });

    private async Task<RunCounts> LoadAndProcessFeed(LibraryTitleIndex index, List<string> errors, CancellationToken cancellationToken)
    {
        ...
        var feedResult = await nyaa.GetLatest(cancellationToken);
        return await feedResult.MatchToValue(
            entries => ProcessEntries(entries, checkpoint, index, cancellationToken),
            error =>
            {
                errors.Add($"Nyaa fetch failed: {error.Message}");
                return Task.FromResult(default(RunCounts));
            });
    }

    private async Task<RunCounts> ProcessEntries(
        ImmutableArray<NyaaEntry> entries, CollectionCheckpoint checkpoint, LibraryTitleIndex index, CancellationToken cancellationToken)
    {
    ...
    private static ImmutableArray<NyaaEntry> DiffAgainstCheckpoint(ImmutableArray<NyaaEntry> entries, CollectionCheckpoint checkpoint) =>
    ...
    private async Task ProcessMatch(MatchedRelease release, CancellationToken cancellationToken)
    {
        ...
        await _upsertConfirmation(newRelease.UpdatedConfirmation, cancellationToken)
            ...
            .AddLogOnFailure(_ => log => log.LogWarning("Failed to persist detected release for series {SeriesId}", release.SeriesId))
    ...
    private Task PromoteTrackability(Result<SeriesClassification> classification, int seriesId, CancellationToken cancellationToken) =>
        classification.Match(
            current => current.Trackability == SeriesTrackability.Trackable
                ...
                : _upsertClassification(SeriesClassifier.MarkTrackable(current), cancellationToken)
                    .AddLogOnFailure(_ => log => log.LogWarning("Failed to promote classification for series {SeriesId}", seriesId))
                    .Complete(logger),
