...
{
    private readonly HtmlRenderer _blazorRenderer;
    public BlazorRenderer(HtmlRenderer blazorRenderer)
    {
    ...
    private Task<string> RenderComponent<T>(ParameterView parameters) where T : IComponent
    {
        ...
        return _blazorRenderer.Dispatcher.InvokeAsync(async () =>
        {
            HtmlRootComponent output = await _blazorRenderer.RenderComponentAsync<T>(parameters);
